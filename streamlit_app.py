...
# ─── CORE INITIALIZATION ───
@st.cache_resource
def get_og_client(_pk):
    try:
        return og.Client(private_key=_pk)
    except Exception as e:
        st.error(f"❌ SDK Connection Error: {e}")
        return None
...
if pk:
    ...
    try:
        ...
        st.sidebar.warning(f"Web3 Error: {e}")
else:
...
<style>
    ...
    .panel-title {{ font-weight: 700; font-size: 1rem; margin-bottom: 20px; color: #fff; }}
    ...
    button[kind="secondary"] {{
        ...
        border-radius: 99px !important; padding: 5px 15px !important;
    }}
    button[kind="secondary"]:hover {{ color: #22d3ee !important; border-color: #22d3ee !important; }}
...
<div style="text-align:center; color:#6b7fa8; font-size:1.1rem; margin-top:-10px;">🛡️ AI-powered auditor on OpenGradient TEE</div>
...
EXAMPLES = {
    ...
    "Reentrancy Example": "// SPDX-License-Identifier: MIT\npragma solidity ^0.8.0;\n\ncontract Bank {\n    mapping(address => uint) public balances;\n    function withdraw() public {\n        uint bal = balances[msg.sender];\n        require(bal > 0);\n        (bool sent,) = msg.sender.call{value: bal}(\"\");\n        require(sent);\n        balances[msg.sender] = 0;\n    }\n}"
}
...
with col1:
    ...
    if btn_cols[0].button("ERC-20 Example", key="ex1"): 
        st.session_state.editor = EXAMPLES["ERC-20 Example"]
        st.rerun()
    if btn_cols[1].button("Vulnerable Code", key="ex2"): 
        st.session_state.editor = EXAMPLES["Vulnerable Code"]
        st.rerun()
    if btn_cols[2].button("Reentrancy Example", key="ex3"): 
        st.session_state.editor = EXAMPLES["Reentrancy Example"]
        st.rerun()
...
with col2:
    ...
    if "audit_res" in st.session_state:
        ...
        <div style="background:rgba(0,0,0,0.3); border-radius:15px; padding:30px; text-align:center; border:1px solid rgba(255,255,255,0.05); margin-bottom:1.5rem;">
            <div style="font-size:0.8rem; color:#6b7fa8; margin-bottom:10px;">Security Risk Score</div>
            <div style="font-size:4rem; font-weight:800; color:{s_color}; line-height:1;">{s_val}</div>
            <div style="font-size:0.95rem; margin-top:20px;">{aud.get('summary','')}</div>
        </div>
        """, unsafe_allow_html=True)
        ...
        if st.session_state.get("tx_h"):
            st.markdown(f'<div style="text-align:center; margin-top:15px;"><a href="https://sepolia.basescan.org/tx/{st.session_state.tx_h}" target="_blank" style="color:#22d3ee; font-size:0.8rem;">🔗 View On-Chain Proof</a></div>', unsafe_allow_html=True)
    else:
        st.markdown("""
